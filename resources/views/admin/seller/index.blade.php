<x-app>
    @section('title', 'List Seller')

    @section('body-content')
        <x-sidebar>
            <div class="p-6">
                <h1 class="text-2xl font-bold text-teal-600 mb-6">
                    Halaman Daftar Seller
                </h1>

                {{-- Add Seller --}}
                <div x-data="{ openAddSellerModal: false }">
                    @include('admin.seller.add')
                    <button @click="openAddSellerModal = true"
                        class="inline-flex items-center gap-2 bg-teal-600 hover:bg-teal-700 text-white px-4 py-2 rounded-lg shadow cursor-pointer font-semibold">
                        <i class="bi bi-plus-lg"></i>
                        Tambah Seller
                    </button>
                </div>

                {{-- Seller Table --}}
                <div class="mt-6">
                    <div class="overflow-x-auto">
                        <table class="min-w-full border border-teal-200 rounded-lg overflow-hidden">
                            <thead class="bg-teal-500 text-white">
                                <tr>
                                    <th class="px-4 py-3 text-left">Photo</th>
                                    <th class="px-4 py-3 text-left">Name</th>
                                    <th class="px-4 py-3 text-left">Email</th>
                                    <th class="px-4 py-3 text-left">Level</th>
                                    <th class="px-4 py-3 text-center">Action</th>
                                </tr>
                            </thead>

                            <tbody class="bg-white divide-y divide-teal-100">
                                @foreach ($users as $user)
                                    @if ($user->role === 'seller')
                                        <tr class="hover:bg-teal-50 transition">
                                            {{-- Photo Profile --}}
                                            <td class="px-4 py-3">
                                                @php
                                                    // 1. Buat Inisial
                                                    $initials = collect(explode(' ', $user->name))
                                                        ->map(fn($word) => strtoupper(substr($word, 0, 1)))
                                                        ->take(2)
                                                        ->implode('');

                                                    // 2. Buat URL Gravatar (s=100 untuk ukuran, d=404 agar memicu error jika tidak ada foto)
                                                    $gravatar =
                                                        'https://www.gravatar.com/avatar/' .
                                                        md5(strtolower(trim($user->email))) .
                                                        '?s=100&d=404';
                                                @endphp

                                                <div class="flex items-center justify-center w-10 h-10">
                                                    @if ($user->foto_profile)
                                                        {{-- Prioritas 1: Foto dari Storage --}}
                                                        <img src="{{ asset('storage/' . $user->foto_profile) }}"
                                                            alt="{{ $user->name }}"
                                                            class="w-10 h-10 rounded-full object-cover shadow-sm">
                                                    @else
                                                        {{-- Prioritas 2: Gravatar dengan Fallback ke Inisial --}}
                                                        <img src="{{ $gravatar }}" alt="{{ $user->name }}"
                                                            class="w-10 h-10 rounded-full object-cover shadow-sm"
                                                            onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">

                                                        {{-- Prioritas 3: Inisial (Muncul jika Gravatar 404) --}}
                                                        <div class="hidden w-10 h-10 rounded-full bg-teal-500 text-white flex items-center justify-center font-semibold text-xs uppercase"
                                                            title="{{ $user->name }}">
                                                            {{ $initials }}
                                                        </div>
                                                    @endif
                                                </div>
                                            </td>

                                            <td class="px-4 py-3 font-medium text-gray-800">
                                                {{ $user->name }}
                                            </td>

                                            <td class="px-4 py-3 text-gray-600">
                                                {{ $user->email }}
                                            </td>

                                            <td class="px-4 py-3">
                                                <span class="px-3 py-1 text-sm rounded-full bg-teal-100 text-teal-700">
                                                    {{ ucfirst($user->role) }}
                                                </span>
                                            </td>

                                            {{-- Action Icons --}}
                                            <td class="px-4 py-3 text-center">
                                                <div class="flex items-center justify-center gap-4">

                                                    {{-- Detail --}}
                                                    <div x-data="{ openDetailSellerModal: false }">
                                                        @include('admin.seller.detail', [
                                                            'user' => $user,
                                                        ])
                                                        <button @click="openDetailSellerModal = true" type="button"
                                                            class="text-blue-500 hover:text-blue-600 transition"
                                                            title="Detail">
                                                            <i class="bi bi-eye cursor-pointer"></i>
                                                        </button>
                                                    </div>

                                                    {{-- Edit --}}
                                                    <div x-data="{ openEditSellerModal: false }">
                                                        @include('admin.seller.edit', [
                                                            'user' => $user,
                                                        ])
                                                        <button @click="openEditSellerModal = true" type="button"
                                                            class="text-yellow-500 hover:text-yellow-600 transition"
                                                            title="Edit">
                                                            <i class="bi bi-pencil-square cursor-pointer"></i>
                                                        </button>
                                                    </div>

                                                    {{-- Delete --}}
                                                    @if ($user->books_count > 0)
                                                        <button type="button"
                                                            class="text-gray-300 cursor-not-allowed group relative"
                                                            onclick="alert('Seller tidak bisa dihapus karena masih memiliki {{ $user->books_count }} buku aktif.')">
                                                            <i class="bi bi-trash"></i>
                                                            {{-- Tooltip sederhana --}}
                                                            <span
                                                                class="absolute bottom-full mb-2 hidden group-hover:block bg-gray-800 text-white text-xs p-1 rounded">
                                                                Masih ada {{ $user->books_count }} buku
                                                            </span>
                                                        </button>
                                                    @else
                                                        <form id="deleteSellerForm-{{ $user->id }}" method="POST"
                                                            action="{{ route('admin.sellers.delete', $user->id) }}"
                                                            class="inline">
                                                            @csrf
                                                            @method('DELETE')

                                                            <button type="button"
                                                                onclick="confirmDeleteSeller({{ $user->id }}, '{{ $user->name }}')"
                                                                class="text-red-500 hover:text-red-600 transition"
                                                                title="Delete">
                                                                <i class="bi bi-trash cursor-pointer"></i>
                                                            </button>
                                                        </form>
                                                    @endif
                                                </div>
                                            </td>
                                        </tr>
                                    @endif
                                @endforeach
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </x-sidebar>
    @endsection
</x-app>
